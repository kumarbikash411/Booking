...
class Hotel extends \Magento\Framework\App\Action\Action
{
    /**
    ...
    public function execute()
    {
        ...
        $resultPage->getConfig()->getTitle()->set('Find Hotels');

        $post = $this->getRequest()->getParams();

        if (isset($post['check_in']) && $post['check_in'] == '' && isset($post['check_out']) && $post['check_out'] != '') {
            $this->_redirect('*/*/*/');
            $this->messageManager->addError('Please select check-in date.');
            
        }
        if (isset($post['check_in']) && $post['check_in'] != '' && isset($post['check_out']) && $post['check_out'] == '') {
            $this->_redirect('*/*/*/');
            $this->messageManager->addError('Please select check-out date.');
