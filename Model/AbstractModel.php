...
abstract class AbstractModel extends \Magento\Framework\Model\AbstractModel
{
    ...
    public function zendValidate($field,$value,$classes = '',$isRequired = false) 
    {
        ...
        return $errors;
        if(is_array($value)) {
            $value=isset($value['value'])?$value['value']:count($value) > 0?implode(',', $value):'';
        }
        if(is_array($classes) && count($classes) > 0 && strlen($value)) {    
            foreach ($classes as $class) {
                ...
                case 'validate-email'   : if (!Zend_Validate::is($value, 'EmailAddress')) {
                        $errors[] = __('Invalid email address "%s" in "%s" Field.', $value, $field);
                }
                    break;
        ...
        if ($isRequired && trim($value) == '') {
            $errors[] = __('"%s" is a required field.', $field);
        }
        return $errors;
    }
    ...
    public function checkAvailability($venderData = array(),$id = 0) 
    {
        ...
        if (strlen($shopUrl) && preg_match('/^[a-z0-9][a-z0-9_\/-]+(\.[a-z0-9_-]+)?$/', $shopUrl)) {    
            ...
            if($rawShopUrl != $shopUrl) {
                $json = array('success'=>0,'message'=>__('Please enter a valid Shop URL Key. For example "my-shop-url".'),'shop_url' => $shopUrl,'raw_shop_url'=>$rawShopUrl,'suggestion' => __('Suggested Shop URL').' : <b>'.$shopUrl.'</b>');
            } elseif($id) {
     ...
     */
    public function getUnusedPath($requestPath = '',$count = 0, $suffix = '')
    {
        $tempPath = $requestPath;
        if(strlen($requestPath) == 0) return $requestPath;
        if($count) $requestPath = $requestPath . '-' . $count;
        $existModel = $this->_objectManager->create('Magento\UrlRewrite\Model\UrlRewrite')
                           ->getCollection()
                           ->addFieldToFilter('request_path',array('eq'=>$requestPath.$suffix))
                           ->getFirstItem();
