...
define([
    ...
    var hideButtons = setInterval(function() {
        ...
        if ($('[data-role="selected-option"]').length > 0) {

            if ($('[data-role="selected-option"]').html() == 'Daily Rent Booking' || $('[data-role="selected-option"]').html() == 'Hourly Rent Booking') {
                    $('[data-index="booking"]').show();
                    $('[data-index="room_button"]').hide();
                    $('[data-index="0"]').hide();
            } else {
                    if ($('[data-role="selected-option"]').html() == 'Default') {
                            $('[data-index="booking"]').hide();
                    }
                    if ($('[data-role="selected-option"]').html() == 'Hotel Booking') {
                        $('[data-index="booking"]').show();
        ...
        openModalWithRoomsData: function (rowIndex) {
           ...
           var roomdata = this.source.get(this.dataScope + '.' + this.index + '.' + rowIndex),
            ...
            if (product.id == '0') {
                ...
                alert('Please save room then edit room.');
                return false;
            }
           ...
           if ($("#roompop"+rowIndex).length) {
                ...
                var previewPopup = $('<div/>',{id : 'roompop'+rowIndex,class: 'room_edit_popup' });
                //previewPopup.addClass('room_edit_popup');
                var roompopup = previewPopup.modal({
                    type: 'slide',
                    title: 'Edit Room',
                    class: 'product_form_product_form_bookingModal',
                    innerScroll: true,
                    modalLeftMargin: 15,
                    buttons: [],
                    opened: function(rowIndex) {
