...
class BeforeLoadLayout  implements ObserverInterface
{
    ...
    public function __construct(
        ...
        $this->_feedHelper = $this->_objectManager->get('Ced\Booking\Helper\Feed');
    }
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
       try {
          ...
          //  print_r($this->_objectManager->get('Ced\CsMarketplace\Helper\Feed')->getCedCommerceExtensions(false,true));die;
            ...
            $helper = $this->_objectManager->create('Ced\Booking\Helper\Feed');
            ...
            foreach ($modules as $moduleName=>$releaseVersion)
            {
                ...
                $h = $this->_objectManager->create('Ced\Booking\Helper\Data')->getStoreConfig(\Ced\Booking\Block\Extensions::HASH_PATH_PREFIX.$m.'_hash'); 
         
                for($i=1;$i<=(int)$this->_objectManager->create('Ced\Booking\Helper\Data')->getStoreConfig(\Ced\Booking\Block\Extensions::HASH_PATH_PREFIX.$m.'_level');$i++)
                    {  
                        $h = base64_decode($h);
                    }
              ...
              //  print_r($h);die;
                if(is_array($h) && isset($h['domain']) && isset($h['module_name']) && isset($h['license']) && strtolower($h['module_name']) == $m && $h['license'] == $this->_objectManager->create('Ced\Booking\Helper\Data')->getStoreConfig(\Ced\Booking\Block\Extensions::HASH_PATH_PREFIX.$m))
                 {}else{
            ...
            foreach ($result as $moduleName => $response) {
                if ($response && isset($response['hash']) && isset($response['level'])) {
                    ...
                    try {
                        ...
                        {
                            $path = \Ced\Booking\Block\Extensions::HASH_PATH_PREFIX.strtolower($moduleName).'_hash';
                              $this->_feedHelper->setDefaultStoreConfig($path, $response['hash'], 0);
                            $path = \Ced\Booking\Block\Extensions::HASH_PATH_PREFIX.strtolower($moduleName).'_level';
                             $this->_feedHelper->setDefaultStoreConfig($path, $response['level'], 0);
                            $path = \Ced\Booking\Block\Extensions::HASH_PATH_PREFIX.strtolower($moduleName);
